...
void main() {
  group('UpcomingWaypointsList', () {
    testWidgets('displays list of upcoming waypoints', (tester) async {
      ...
      final waypoints = [
        const UpcomingWaypoint(name: 'Water Source', distanceKm: 2.1, type: WaypointType.water, bearing: 90, alertCount: 3, notes: ''),
        const UpcomingWaypoint(name: 'Trailhead', distanceKm: 5.4, type: WaypointType.trailhead, bearing: 120, alertCount: 1, notes: ''),
      ];
    ...
    testWidgets('shows empty state when no waypoints', (tester) async {
      ...
      expectTextNotFound('UPCOMING (within ${maxDistanceKm.toStringAsFixed(1)} km)');
    });

    testWidgets('display alert count chips', (tester) async {
      final waypoints = [
        const UpcomingWaypoint(name: 'Camp Site', distanceKm: 8.7, type: WaypointType.camp, bearing: 310, alertCount: 3, notes: ''),
      ];
